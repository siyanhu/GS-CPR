...
from gaussian_renderer_train import prefilter_voxel, render, network_gui
import sys
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    if args.dataset_type == 'Cambridge':
        bds = load_Cambridge_dataloader_GS(args)
        near = bds[0]
        far = bds[1]
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.encode_hist:
            ...
            rgb = affine_color_transform(args, image, hist, 1)
        image = rgb.reshape(3,900,1600)
        '''
        if not os.path.exists('./render_train/'):
            # 如果目录不存在，创建目录
            os.makedirs('./render_train/')
        '''
        
        
        if args.render_scene == "KingsCollege" or "ShopFacade":
            img_name = viewpoint_cam.image_name.replace('/','_')
            mask_path = f"./data/Cambridge_semantic/{args.render_scene}_semantic_train/" + img_name
            mask = Image.fromarray(np.array(Image.open(mask_path)).astype("uint8"))
            height, width = gt_image.shape[1:]
            target_shape = (width, height)
            mask = (np.array(mask) / 255.).astype(np.float32)
            mask = cv2.resize(mask, target_shape, interpolation=cv2.INTER_AREA)
            mask = (mask>0.5).astype(np.float32)
            mask = torch.Tensor(mask)
            mask = mask.unsqueeze(0).repeat(3, 1, 1).cuda()
        ...
        with torch.no_grad():
            # Progress bar
            #if iteration % 1000 == 0:
            #    torchvision.utils.save_image(image, './render_train/' + viewpoint_cam.image_name.replace('/frame','_frame').replace('.jpg','') + '_after_act_' + str(iteration) + '.jpg')
            ...
            if iteration%args.i_weights==0 and iteration!=0:
                path = os.path.join(basedir, expname, args.render_scene , '{:06d}.tar'.format(iteration))
                ...
                if not os.path.exists(os.path.join(basedir, expname, args.render_scene)):
                    os.makedirs(os.path.join(basedir, expname, args.render_scene))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[15_000])
