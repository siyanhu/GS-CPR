...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        '''
        gt_image = viewpoint_cam.original_image.cuda()* 255
        gt_image = gt_image.to(torch.uint8)  
        '''
        ...
        loss.backward()
        #print(f"Gradient of gs grad: {image.grad}")
        iter_end.record()
        ...
        with torch.no_grad():
            ...
            #if iteration % 2000 == 0:
            #    torchvision.utils.save_image(image, './render_train/' + viewpoint_cam.image_name.replace('/frame','_frame').replace('.jpg','') + str(iteration) + '.jpg')
